...
annotation_dir_path = os.path.join(script_dir, '../data/knee/pixel_labels')
image_dir_path = os.path.join(script_dir, '../data/knee/train_images')
save_model_path = os.path.join(script_dir, '../final_results/knee/C_marl/model_savev5')
...
class DQN(nn.Module):
    def __init__(self, config):
        ...
        self.fc1 = nn.ModuleList([
            nn.Linear(1024, 256) for _ in range(self.num_landmarks)
        ])
        self.prelu4 = nn.ModuleList([
            nn.PReLU() for _ in range(self.num_landmarks)
        ])
        self.fc2 = nn.ModuleList([
            nn.Linear(256, 64) for _ in range(self.num_landmarks)
        ])
...
class LandmarkEnv:
    ...
    def reset(self, idx=None):
        ...
        self.steps = 0
        self.terminated_results = [None for _ in range(self.num_landmarks)]  # 终止状态记录
    ...
    def _get_patch(self, position):
        ...
        add=0
        #如果self.half_patch为奇数
        if self.half_patch % 2 == 1:
            add = 1
        ...
        if (x < self.half_patch or x >= self.image_size - self.half_patch or
            ...
            return padding_image[torch.newaxis,y:y+2*self.half_patch+add, 
                         x:x+2*self.half_patch+add]
        return self.image[:1, y-self.half_patch:y+self.half_patch+add, 
                         x-self.half_patch:x+self.half_patch+add]
    ...
    def step(self, actions, mode="test"):
        """执行动作，返回新状态、奖励、终止标志，以及所有动作的可能下一状态和奖励"""
            ...
            for i in range(self.num_landmarks):
                ...
                for action in range(4):
                    ...
                    if x < 0 or x >= self.image_size or y < 0 or y >= self.image_size:
                        ...
                        sim_seq = torch.zeros(self.config['memory_length'], 1, self.patch_size, self.patch_size)
                    else:
                        ...
                        sim_seq = torch.stack(list(sim_queue), dim=0)
                    all_next_states[i, :, action, :, :] = sim_seq.squeeze(1)
                self.positions[i] = next_positions[i]

            # 新增：保存终止点的step结果
            for i in range(self.num_landmarks):
                if self.terminated[i] and self.terminated_results[i] is None:
                    # 保存本次step的结果
                    self.terminated_results[i] = {
                        'next_state_seq': next_state_seq.clone(),
                        'rewards': rewards.clone(),
                        'terminated': self.terminated.clone(),
                        'all_next_states': all_next_states.clone(),
                        'all_rewards': all_rewards.clone(),
                        'reachbound': reachbound.clone()
                    }

            # 新增：如果有终止点，直接用保存的结果覆盖
            # 只对终止点覆盖结果
            for i in range(self.num_landmarks):
                if self.terminated[i] and self.terminated_results[i] is not None:
                    # 用保存的结果覆盖对应项
                    next_state_seq[i] = self.terminated_results[i]['next_state_seq'][i]
                    rewards[i] = self.terminated_results[i]['rewards'][i]
                    all_next_states[i] = self.terminated_results[i]['all_next_states'][i]
                    all_rewards[i] = self.terminated_results[i]['all_rewards'][i]
                    reachbound[i] = self.terminated_results[i]['reachbound'][i]
...
class LandmarkTrainer:
    ...
    def repeated_single_test(self, sample_idx=None, repeats=20, plot=False):
        """
        单样本重复测试方法
        :param sample_idx: 样本索引，None表示随机选择
        :param repeats: 重复次数
        :param plot: 是否绘制可视化结果
        :return: 最终预测位置与金标准的欧氏距离误差
        """
        # 存储每次运行的结果位置
        all_positions = []
        ...
        self.agent.eps = 0
        print(f"\nStarting repeated test for sample {sample_idx} with {repeats} repeats...")
        for run in range(repeats):
            ...
            # 运行直到所有点终止
            while not all(self.env.terminated) and step < self.config['episode_steps']:
                action = self.agent.select_action(state, reachbound, explore=False)
                next_state, reward, done, reachbound = self.env.step(action)
                state = next_state
                print(f"step {step},{self.env.positions.tolist()}")
                step+=1
                
            
            # 记录最终位置
            final_positions = self.env.positions.clone().cpu().numpy()
        ...
        # 计算平均位置
        all_positions = np.array(all_positions)  # (repeats, num_landmarks, 2)
        mean_positions = np.mean(all_positions, axis=0)
        
        # 获取金标准位置
        target_positions = self.env.landmarks.cpu().numpy()
        
        # 计算每个关键点的最终误差
        errors = np.linalg.norm(mean_positions - target_positions, axis=1)
        
        # 可视化结果
        if plot:
            self._plot_repeated_test_results(
                self.env.image, 
                target_positions, 
                all_positions, 
                mean_positions
    ...
    def _plot_repeated_test_results(self, image_tensor, targets, all_positions, mean_positions):
        """
        绘制重复测试结果的可视化
        """
        # 准备图像
        ...
        for i, (x, y) in enumerate(targets):
            plt.scatter(x, y, s=100, c='green', marker='o', alpha=0.8)
            plt.text(x+5, y+5, f'Target {i}', fontsize=10, color='green')
        
        # 绘制每次运行的结果（半透明黄色叉）
        for run in all_positions:
            for i, (x, y) in enumerate(run):
                plt.scatter(x, y, s=60, c='yellow', marker='x', alpha=0.3)
        
        # 绘制最终平均结果（红色叉）
        for i, (x, y) in enumerate(mean_positions):
            plt.scatter(x, y, s=100, c='red', marker='x')
            plt.text(x+5, y+5, f'Pred {i}', fontsize=10, color='red')
        
        # 连接重复结果与平均结果（橙色细实线）
        for run in all_positions:
            for i, pos in enumerate(run):
                plt.plot(
                    [pos[0], mean_positions[i][0]], 
                    [pos[1], mean_positions[i][1]], 
                    'orange', linewidth=0.5, alpha=0.3
                )
        
        # 连接金标准与平均结果（蓝色虚线）
        for i, (target, mean) in enumerate(zip(targets, mean_positions)):
            plt.plot(
                [target[0], mean[0]], 
                [target[1], mean[1]], 
                'b--', linewidth=1.5
            ...
            # 添加距离文本
            distance = np.linalg.norm(target - mean)
            mid_x = (target[0] + mean[0]) / 2
            mid_y = (target[1] + mean[1]) / 2
            plt.text(mid_x, mid_y, f'{distance:.1f}px', fontsize=9, 
                     bbox=dict(facecolor='white', alpha=0.7))
        
        plt.imshow(image, cmap='gray' if image.shape[2]==1 else None)
        plt.title(f'Repeated Test Results (n={len(all_positions)})')
        plt.axis('off')
        plt.tight_layout()
        plt.savefig(os.path.join(self.save_path, 'repeated_test_results.png'))
    
    def test_dataset(self, dataset, repeats=20):
        """
        测试整个数据集
        :param dataset: BaseLandmarkDataset实例
        :param repeats: 每个样本的重复次数
        :return: 包含所有样本最终预测误差的一维数组
        """
        # 保存原始epsilon值，测试时设为0
        original_eps = self.agent.eps
        self.agent.eps = 0
        ...
        # 进度条
        pbar = tqdm(total=len(dataset), desc="Testing Dataset")
        ...
        for idx in range(len(dataset)):
            ...
            reachbound = None
            step=0
            print(f"\nTesting sample {idx+1}/{len(dataset)}")
            # 存储当前样本的多次运行位置
            sample_positions = []
            ...
            for run in range(repeats):
                ...
                while not all(self.env.terminated) and step < self.config['episode_steps']:
                    ...
                    next_state, reward, done, reachbound = self.env.step(action)
                    state = next_state
                    print(f"Testing sample {idx+1}/{len(dataset)}, step {step}")
                    step+=1
                
                # 记录最终位置
                final_positions = self.env.positions.clone().cpu().numpy()
                sample_positions.append(final_positions)
                
                # 重置环境（同一图像）
                state = self.env.reset(idx)
                reachbound = None
                step=0
                print(f"Tested sample {idx+1}/{len(dataset)}, repeat {run+1}/{repeats}")
            
            # 计算平均位置
            sample_positions = np.array(sample_positions)  # (repeats, num_landmarks, 2)
            mean_positions = np.mean(sample_positions, axis=0)
            ...
            # 更新进度条
            pbar.update(1)
            pbar.set_postfix({'Avg Error': f'{np.mean(all_errors):.2f}px'})
...
if __name__ == "__main__":
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    annotation_dir_path = os.path.join(script_dir, '../data/knee/pixel_labels')
    image_dir_path = os.path.join(script_dir, '../data/knee/val_images')
    os.makedirs(save_model_path, exist_ok=True)
    log_model_path = os.path.join(save_model_path, 'log')
    os.makedirs(log_model_path, exist_ok=True)

    # 创建数据集实例
    val_dataset = BaseLandmarkDataset(
        load_image_dir=image_dir_path,
        load_annotation_dir=annotation_dir_path,
        target_size=(config['image_size'], config['image_size']),
        num_landmarks=config['num_landmarks'],
    ...
    final_model_path = os.path.join(save_model_path, "dqn_model_ep3400.pth")
    trainer.agent.load_model(final_model_path)
    # dataset_errors = trainer.test_dataset(val_dataset, repeats=20)
    # print(f"数据集平均误差: {np.mean(dataset_errors):.2f}px")
    # print(f"最大误差: {np.max(dataset_errors):.2f}px")
    # print(f"最小误差: {np.min(dataset_errors):.2f}px")

    # # 保存结果
    # np.savetxt("C_marl_chest_6.txt", dataset_errors)
    errors = trainer.repeated_single_test(sample_idx=40, repeats=20, plot=True)
    print(f"单样本关键点误差: {errors}")