...
class ResNet_NOFPN(nn.Module):
    def __init__(self, arch, block, layers, output_stride, BatchNorm, pretrained=True):
        self.inplanes = 64
        self.layers = layers
        self.arch = arch
        super(ResNet_NOFPN, self).__init__()
        blocks = [1, 2, 4]
        if output_stride == 16:
            strides = [1, 2, 2, 1]
            dilations = [1, 1, 1, 2]
...
def FPN101_NO(output_stride, BatchNorm=nn.BatchNorm2d, pretrained=True):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_NOFPN('resnet101', Bottleneck, [3, 4, 23, 3], output_stride, BatchNorm, pretrained=pretrained)
    return model
...
def FPN50_NO(output_stride, BatchNorm=nn.BatchNorm2d, pretrained=True):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_NOFPN('resnet50', Bottleneck, [3, 4, 6, 3], output_stride, BatchNorm, pretrained=pretrained)
    return model
...
def FPN18_NO(output_stride, BatchNorm=nn.BatchNorm2d, pretrained=True):
    model = ResNet_NOFPN('resnet18', BasicBlock, [2, 2, 2, 2], output_stride, BatchNorm, pretrained=pretrained)
    return model
