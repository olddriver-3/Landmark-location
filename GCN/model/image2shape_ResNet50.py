...
from .backbone import FPN101, FPN50, FPN18, ResNext50_FPN
...
#         transform_matrix[:, 4].view(-1, 1) * vertices[:, :, 1] + \
#         transform_matrix[:, 5].view(-1, 1)
#     w = transform_matrix[:, 6].view(-1, 1) * vertices[:, :, 0] + \
#         transform_matrix[:, 7].view(-1, 1) * vertices[:, :, 1] + \
#         transform_matrix[:, 8].view(-1, 1)
#     transform_vertices = torch.stack([x / w, y / w], dim=-1)
#     return transform_vertices
#
...
class Image2Shape(nn.Module):
    def __init__(self, config):
        ...
        """GCN图卷积的框架层"""
        # 第一个特征采样
        self.graph_sample1 = PatternSampling(features_count=1024)
        self.up_f2f_layer1 = Features2Features(in_features=1024 + 2, out_features=128, hidden_layer_count=1)
        # self.up_f2v_layer1 = Feature2VertexLayer(in_features=128, hidden_layer_count=1)
        self.up_f2v_layer1 = nn.Linear(in_features=128, out_features=6)
        # 第二个特征采样
        self.graph_sample2 = PatternSampling(features_count=1024)
        self.up_f2f_layer2 = Features2Features(in_features=1024 + 2, out_features=128, hidden_layer_count=1)
        self.up_f2v_layer2 = Feature2VertexLayer(in_features=128, hidden_layer_count=1)
        # 第三个特征采样
        self.graph_sample3 = PatternSampling(features_count=1024)
        self.up_f2f_layer3 = Features2Features(in_features=1024 + 2, out_features=128, hidden_layer_count=1)
        self.up_f2v_layer3 = Feature2VertexLayer(in_features=128, hidden_layer_count=1)
        # 第四个特征采样
        self.graph_sample4 = PatternSampling(features_count=1024)
        self.up_f2f_layer4 = Features2Features(in_features=1024 + 2, out_features=128, hidden_layer_count=1)
        self.up_f2v_layer4 = Feature2VertexLayer(in_features=128, hidden_layer_count=1)
        # 第五个特征采样 -> unet_encoder4
        self.graph_sample5 = PatternSampling(features_count=1024)
        self.up_f2f_layer5 = Features2Features(in_features=1024 + 2, out_features=128, hidden_layer_count=1)
        self.up_f2v_layer5 = Feature2VertexLayer(in_features=128, hidden_layer_count=1)
        # 第六个特征采样 -> unet_encoder4
        self.graph_sample6 = PatternSampling(features_count=1024)
        self.up_f2f_layer6 = Features2Features(in_features=1024 + 2, out_features=128, hidden_layer_count=1)
